...
function compareResults(baseResults: any, comparisonResults: any, title: string): void {
  console.log(`\n>>> ${title} <<<`);
  console.log(
    `Monthly Payment Difference: ${(comparisonResults.monthlyPayment - baseResults.monthlyPayment).toFixed(2)} PLN`
  );
  console.log(
    `Total Interest Difference: ${(comparisonResults.totalInterest - baseResults.totalInterest).toFixed(2)} PLN`
  );
  console.log(
    `Term Difference: ${(comparisonResults.actualTerm - baseResults.actualTerm).toFixed(2)} years`
  );
}
...
describe('Overpayment Calculation Fix Tests', () => {
  ...
  test('Multiple interest rate periods: 19 years at 4.5% + 11 years at 20%', async () => {
    ...
    // Calculate loan details
    const results = await calculateLoanDetails(principal, interestRatePeriods, termYears);

    // Log detailed information
    logSummary('Multiple Interest Rate Periods Results', results);
    logPaymentDetails('First Few Payments', results.amortizationSchedule, 0, 5);
    logPaymentDetails(
      'Payments Around Rate Change',
  ...
  test('One-time overpayment of 1,000 PLN at month 6', async () => {
    ...
    compareResults(
      standardResults,
      overpaymentResults,
      'Impact of 1,000 PLN Overpayment at Month 6'
    );

    // Assertions
    expect(overpaymentResults.amortizationSchedule[overpaymentMonth - 1].isOverpayment).toBe(true);
    expect(overpaymentResults.amortizationSchedule[overpaymentMonth - 1].overpaymentAmount).toBe(
      overpaymentAmount
    );
    ...
    expect(overpaymentResults.amortizationSchedule[overpaymentMonth - 1].balance).toBeCloseTo(
      expectedBalanceAfterOverpayment,
      2
    );

    // Verify subsequent payments have correct balance progression
    for (
      let i = overpaymentMonth;
      i < Math.min(overpaymentMonth + 5, overpaymentResults.amortizationSchedule.length);
      i++
    ) {
      expect(overpaymentResults.amortizationSchedule[i].balance).toBeLessThan(
        standardResults.amortizationSchedule[i].balance
      );
    }
  });
  ...
  test('One-time overpayment of 1,000 PLN at month 12', async () => {
    ...
    compareResults(
      standardResults,
      overpaymentResults,
      'Impact of 1,000 PLN Overpayment at Month 12'
    );
  ...
  test('One-time overpayment of 1,000 PLN at month 24', async () => {
    ...
    compareResults(
      standardResults,
      overpaymentResults,
      'Impact of 1,000 PLN Overpayment at Month 24'
    );
  ...
  test('Monthly overpayment of 1,000 PLN', async () => {
    ...
    logPaymentDetails(
      'First Few Payments with Monthly Overpayment',
      overpaymentResults.amortizationSchedule,
      0,
      5
    );
    ...
    for (let i = 0; i < 5; i++) {
      ...
      expect(overpaymentResults.amortizationSchedule[i].overpaymentAmount).toBe(
    ...
    for (let i = 0; i < Math.min(10, overpaymentResults.amortizationSchedule.length); i++) {
      if (i > 0) {
        expect(overpaymentResults.amortizationSchedule[i].balance).toBeLessThan(
          overpaymentResults.amortizationSchedule[i - 1].balance
        );
      }
      expect(overpaymentResults.amortizationSchedule[i].balance).toBeLessThan(
        standardResults.amortizationSchedule[i].balance
  ...
  test('Combination of one-time and monthly overpayments', async () => {
    ...
    const combinedResults = {
      amortizationSchedule: performOverpayments(
        schedule,
        overpaymentPlans,
    ...
    const lastPositiveBalanceIndex = combinedResults.amortizationSchedule.findIndex(
      (p) => p.balance <= 0
    );
    combinedResults.actualTerm =
      (lastPositiveBalanceIndex === -1
    ...
    logPaymentDetails(
      'Payments Around One-time Overpayment',
      combinedResults.amortizationSchedule,
      oneTimeOverpaymentMonth - 2,
      oneTimeOverpaymentMonth + 2
    );
    ...
    for (let i = 0; i < 5; i++) {
      ...
      expect(combinedResults.amortizationSchedule[i].overpaymentAmount).toBe(
        monthlyOverpaymentAmount
      );
    }
    ...
    expect(
      combinedResults.amortizationSchedule[oneTimeOverpaymentMonth - 1].overpaymentAmount
  ...
  test('One-time overpayment with payment reduction', async () => {
    ...
    compareResults(
      standardResults,
      overpaymentResults,
      'Impact of 10,000 PLN Overpayment with Payment Reduction'
    );
    ...
    expect(overpaymentResults.amortizationSchedule.length).toBe(
      standardResults.amortizationSchedule.length
    );
    ...
    for (
      ...
      expect(overpaymentResults.amortizationSchedule[i].monthlyPayment).toBeLessThan(
        standardResults.amortizationSchedule[i].monthlyPayment
      );
    }
  });
  ...
  test('Large one-time overpayment (50% of remaining balance)', async () => {
    ...
    const remainingBalanceAtMonth60 =
      standardResults.amortizationSchedule[overpaymentMonth - 1].balance;
    ...
    compareResults(
      standardResults,
      overpaymentResults,
      'Impact of Large Overpayment (50% of Remaining Balance)'
    );

    // Assertions
    expect(overpaymentResults.amortizationSchedule[overpaymentMonth - 1].isOverpayment).toBe(true);
    expect(
      overpaymentResults.amortizationSchedule[overpaymentMonth - 1].overpaymentAmount
  ...
  test('Multiple interest rate periods with overpayments', async () => {
    ...
    compareResults(
      standardResults,
      overpaymentResults,
      'Impact of Overpayment with Multiple Interest Rate Periods'
    );

    // Log overpayment details for debugging
    console.log('Overpayment month:', overpaymentMonth);
    console.log(
      'Payment at overpayment month:',
      overpaymentResults.amortizationSchedule[overpaymentMonth - 1]
    );
