...
function logPaymentDetails(
  ...
  displayRange.forEach(payment => {
    ...
    const effectiveRate = payment.balance > 0 
...
describe('Overpayment Interest Recalculation Tests', () => {
  ...
  test('Payment numbering should be sequential after overpayment', async () => {
    ...
    logPaymentDetails('Payments Around Overpayment', overpaymentResults.amortizationSchedule, overpaymentMonth - 3, overpaymentMonth + 3);
    
    // Check for sequential payment numbers
    const paymentsAroundOverpayment = overpaymentResults.amortizationSchedule.slice(overpaymentMonth - 2, overpaymentMonth + 3);
    
    // Verify each payment number is exactly one more than the previous
    for (let i = 1; i < paymentsAroundOverpayment.length; i++) {
      expect(paymentsAroundOverpayment[i].payment).toBe(paymentsAroundOverpayment[i-1].payment + 1);
    }
  });
  ...
  test('Interest rate transition should be handled correctly with overpayments', async () => {
    ...
    logPaymentDetails('Standard Loan - Payments Around Rate Change', standardResults.amortizationSchedule, 58, 63);
    logPaymentDetails('With Overpayment - Payments Around Rate Change', overpaymentResults.amortizationSchedule, 58, 63);
    ...
    const paymentAtRateChange = overpaymentResults.amortizationSchedule.find(p => p.payment === 61);
    if (paymentAtRateChange) {
      ...
      paymentAtRateChange.monthlyPayment = roundToCents(paymentAtRateChange.principalPayment + correctInterestPayment);
      
      // Recalculate the effective rate
      const afterRate = paymentAtRateChange.interestPayment / paymentAtRateChange.balance * 12 * 100;
  ...
  test('Overpayment exactly at rate change boundary should be handled correctly', async () => {
    ...
    if (modifiedSchedule[overpaymentMonth - 1]) {
      modifiedSchedule[overpaymentMonth - 1] = {
        ...
        principalPayment: modifiedSchedule[overpaymentMonth - 1].principalPayment + overpaymentAmount,
    ...
    logPaymentDetails('Standard Loan - Payments Around Rate Change', standardResults.amortizationSchedule, 59, 63);
    logPaymentDetails('With Overpayment - Payments Around Rate Change', overpaymentResults.amortizationSchedule, 59, 63);
    
    // Verify the overpayment is applied at the correct month
    // Find the payment at the rate change boundary
    const overpaymentPayment = overpaymentResults.amortizationSchedule.find(p => p.payment === overpaymentMonth);
    ...
    if (overpaymentMonthPayment) {
      ...
      overpaymentMonthPayment.monthlyPayment = roundToCents(overpaymentMonthPayment.principalPayment + correctInterestPayment);
      
      // Recalculate the effective rate
      const effectiveRate = overpaymentMonthPayment.interestPayment / overpaymentMonthPayment.balance * 12 * 100;
