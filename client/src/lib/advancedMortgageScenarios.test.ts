...
describe('Advanced Mortgage Scenarios', () => {
  test('Scenario 1: Check initial payment calculation only', async () => {
    ...
    const expectedInitialPayment = calculateBaseMonthlyPayment(
      initialPrincipal,
      monthlyRate,
      totalMonths
    );
    ...
    const result = await calculateComplexScenario(loanDetails, [], []);
  ...
  test('Scenario 2: Decreasing rate with lump sum payment', async () => {
    ...
    const resultWithOverpayment = await calculateComplexScenario(
      loanDetails,
      rateChanges,
      overpayments
    );
    ...
    expect(resultWithOverpayment.totalInterest).toBeLessThan(
      resultWithoutOverpayment.totalInterest * 0.95
    );
    ...
    expect(paymentAfterDrop).toBeCloseTo(2408.42, 0);
    ...
    console.log(
      'New term after overpayment:',
      resultWithOverpayment.actualTerm.toFixed(2),
      'years'
    );
    console.log(
      'Interest saved:',
  ...
  test('Scenario 3: Increasing rate with bi-weekly payments', async () => {
    ...
    const rateChanges = [{ month: 60, newRate: 4.5 }];
    ...
    const resultWithoutExtraPayments = await calculateComplexScenario(loanDetails, rateChanges, []);
    ...
    console.log(
      'Term with rate change only:',
      formatTimePeriod(resultWithoutExtraPayments.actualTerm * 12)
    );
    console.log('Term with bi-weekly payments:', formatTimePeriod(result.actualTerm * 12));
    console.log(
      'Time saved:',
      formatTimePeriod((resultWithoutExtraPayments.actualTerm - result.actualTerm) * 12)
    );
    console.log(
      'Interest saved:',
      formatCurrency(resultWithoutExtraPayments.totalInterest - result.totalInterest)
    );
  });
});
