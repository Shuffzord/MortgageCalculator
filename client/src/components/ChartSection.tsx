...
export default function ChartSection({
  ...
  const [activeChart, setActiveChart] = useState<
    'pie' | 'bar' | 'barMonthly' | 'line' | 'area' | 'comparison'
  >('pie');
  ...
  useEffect(() => {
              ...
              callbacks: {
                label: function (context) {
                  ...
                  return (
                    ...
                    ': ' +
                    formatCurrency(value, undefined, loanDetails.currency)
                  );
                },
              },
            },
    ...
    const areaChartTimer = setTimeout(() => {
      ...
      const labels =
        timeFrame === 'yearly'
          ? data.map(
              (item) => `Year ${(item as any).year || Math.ceil((item as any).payment / 12)}`
            )
      ...
      const principalData = data.map((item) => {
        cumulativePrincipal +=
          timeFrame === 'yearly' ? (item as any).principal : (item as any).principalPayment;
        return cumulativePrincipal;
      });

      const interestData = data.map((item) => {
        cumulativeInterest +=
          timeFrame === 'yearly' ? (item as any).interest : (item as any).interestPayment;
  ...
  const handlePrevChart = () => {
    setActiveChart((current) => {
      switch (current) {
        case 'pie':
          return comparisonResults?.length ? 'comparison' : 'area';
        case 'bar':
          return 'pie';
        case 'barMonthly':
          return 'bar';
    ...
    setActiveChart((current) => {
      switch (current) {
        ...
        case 'line':
          return 'area';
        case 'area':
          return comparisonResults?.length ? 'comparison' : 'pie';
        case 'comparison':
          return 'pie';
        default:
          return 'bar';
    ...
    <Card>
      <CardContent className="p-6">
        <div className="flex flex-col gap-4 mb-6">
          <div className="flex items-center justify-between">
            ...
            <div className="flex items-center gap-4">
              ...
              <span className="text-sm font-medium">{chartTitles[activeChart]}</span>
        ...
        <div className="relative overflow-hidden">
          <div
            className={`aspect-[16/9] w-full min-h-[400px] bg-white p-4 rounded-lg shadow-sm transition-opacity duration-300 ${activeChart === 'pie' ? 'opacity-100' : 'opacity-0 hidden'}`}
          >
            <canvas ref={pieChartRef}></canvas>
          </div>

          <div
            className={`aspect-[16/9] w-full min-h-[400px] bg-white p-4 rounded-lg shadow-sm transition-opacity duration-300 ${activeChart === 'bar' ? 'opacity-100' : 'opacity-0 hidden'}`}
          >
            <canvas ref={barChartRef}></canvas>
          </div>

          <div
            className={`aspect-[16/9] w-full min-h-[400px] bg-white p-4 rounded-lg shadow-sm transition-opacity duration-300 ${activeChart === 'barMonthly' ? 'opacity-100' : 'opacity-0 hidden'}`}
          >
            <canvas ref={barMonthlyChartRef}></canvas>
          </div>

          <div
            className={`aspect-[16/9] w-full min-h-[400px] bg-white p-4 rounded-lg shadow-sm transition-opacity duration-300 ${activeChart === 'line' ? 'opacity-100' : 'opacity-0 hidden'}`}
          >
            <canvas ref={lineChartRef}></canvas>
          </div>

          <div
            className={`aspect-[16/9] w-full min-h-[400px] bg-white p-4 rounded-lg shadow-sm transition-opacity duration-300 ${activeChart === 'area' ? 'opacity-100' : 'opacity-0 hidden'}`}
          >
          ...
          {comparisonResults && comparisonResults.length > 0 && (
            <div
              className={`aspect-[16/9] w-full min-h-[400px] bg-white p-4 rounded-lg shadow-sm transition-opacity duration-300 ${activeChart === 'comparison' ? 'opacity-100' : 'opacity-0 hidden'}`}
            >
