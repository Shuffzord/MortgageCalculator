...
export function exportToCSV(
  ...
  if (options.includeAmortizationSchedule) {
    ...
    if (options.dateRange) {
      scheduleToExport = scheduleToExport.filter(
        (payment) =>
          payment.payment >= options.dateRange!.startMonth &&
    ...
    scheduleToExport.forEach((payment) => {
      const date = payment.paymentDate ? payment.paymentDate.toISOString().split('T')[0] : '';
      ...
      const row = columns.map((col) => {
        switch (col) {
          ...
          case 'date':
            return date;
          case 'monthlyPayment':
            return formatCurrency(payment.monthlyPayment, undefined, currency);
          case 'principalPayment':
            return formatCurrency(payment.principalPayment, undefined, currency);
          case 'interestPayment':
            return formatCurrency(payment.interestPayment, undefined, currency);
          case 'balance':
            return formatCurrency(payment.balance, undefined, currency);
          case 'totalInterest':
            return formatCurrency(payment.totalInterest, undefined, currency);
          case 'overpaymentAmount':
            return formatCurrency(payment.overpaymentAmount || 0, undefined, currency);
          case 'fees':
            return formatCurrency(payment.fees || 0, undefined, currency);
          default:
            return '';
        }
      });
  ...
  if (options.includeComparisonData && comparisonData) {
    ...
    if (comparisonData.differences.length > 0) {
      csv += '\nDifferences\n';
      csv +=
        'Monthly Payment Difference,Total Interest Difference,Term Difference,Total Cost Difference\n';
...
export function exportToJSON(
    ...
    summary: {
      ...
      formattedValues: {
        ...
        totalCost: formatCurrency(
          loanDetails.principal + results.totalInterest,
          undefined,
          currency
        ),
      },
  ...
  if (results.oneTimeFees || results.recurringFees) {
    ...
    exportData.summary.formattedValues.oneTimeFees = formatCurrency(
      results.oneTimeFees || 0,
      undefined,
      currency
    );
    exportData.summary.formattedValues.recurringFees = formatCurrency(
    ...
    scheduleToExport = scheduleToExport.map((payment) => ({
      ...
      formattedValues: {
        ...
        overpaymentAmount: payment.overpaymentAmount
          ? formatCurrency(payment.overpaymentAmount, undefined, currency)
          : '',
    ...
    if (options.selectedColumns && options.selectedColumns.length > 0) {
      scheduleToExport = scheduleToExport.map((payment) => {
        ...
        options.selectedColumns!.forEach((col) => {
          if (
            col in payment &&
            payment.formattedValues &&
            payment.formattedValues[col as keyof typeof payment.formattedValues]
          ) {
            filteredPayment.formattedValues[col] =
              payment.formattedValues[col as keyof typeof payment.formattedValues];
          }
        });
...
export async function exportToPDF(
  ...
  if (options.includeAmortizationSchedule) {
    ...
    limitedSchedule.forEach((payment) => {
      const date = payment.paymentDate ? payment.paymentDate.toISOString().split('T')[0] : 'N/A';
  ...
  try {
    ...
    if (parsed.summary) {
      ...
      if (parsed.amortizationSchedule && Array.isArray(parsed.amortizationSchedule)) {
        const amortizationSchedule: PaymentData[] = parsed.amortizationSchedule.map(
          (payment: any) => ({
            ...payment,
...
function findSectionIndex(lines: string[], possibleHeaders: string[]): number {
  return lines.findIndex((line) =>
    possibleHeaders.some((header) => line.toLowerCase().includes(header.toLowerCase()))
  );
}
 ...
 */
function calculatePrincipalFromPayment(
...
export function importFromCSV(csvData: string): {
  ...
  try {
    ...
    if (summaryStartIndex >= 0) {
      ...
      for (let i = summaryStartIndex + 1; i < lines.length; i++) {
        ...
        if (key === 'Principal' && typeof parsedValue === 'number') {
          ...
          (key === 'Loan Term' && typeof parsedValue === 'number') ||
    ...
    if (loanDetails.interestRatePeriods && !loanDetails.interestRatePeriods[0].interestRate) {
        ...
        if (payment.monthlyPayment !== undefined) {
          payment.totalPayment =
            payment.payment === 1
              ...
              : (amortizationSchedule[amortizationSchedule.length - 1]?.totalPayment || 0) +
                payment.monthlyPayment;
        }
