...
try {
  ...
  if (!interestRatePeriods || interestRatePeriods.length === 0) throw new Error('Interest rate periods must not be empty');
  for (const period of interestRatePeriods) {
    if (period.interestRate < 0) throw new Error('Interest rate cannot be negative');
  }
