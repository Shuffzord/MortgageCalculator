  ...
  return (
    <Card>
      <CardContent className="p-6">
        <h2 className="text-lg font-semibold text-gray-900 mb-4">{t('comparison.title') || 'Scenario Comparison'}</h2>
        
        <div className="space-y-4">
          {/* Scenario selection */}
          <div className="space-y-2">
            <h3 className="text-sm font-medium">{t('comparison.selectScenarios') || 'Select Scenarios to Compare'}</h3>
          ...
          {/* Comparison options */}
          <div className="space-y-2">
            <h3 className="text-sm font-medium">{t('comparison.options') || 'Comparison Options'}</h3>
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-2">
              <div className="flex items-center space-x-2">
                ...
                />
                <Label htmlFor="option-breakeven">{t('comparison.breakEvenAnalysis') || 'Break-even Analysis'}</Label>
              </div>
              ...
              <div className="flex items-center space-x-2">
                ...
                />
                <Label htmlFor="option-amortization">{t('comparison.amortizationComparison') || 'Amortization Comparison'}</Label>
              </div>
              ...
              <div className="flex items-center space-x-2">
                ...
                />
                <Label htmlFor="option-monthly">{t('comparison.monthlyPaymentComparison') || 'Monthly Payment Comparison'}</Label>
              </div>
              ...
              <div className="flex items-center space-x-2">
                ...
                />
                <Label htmlFor="option-total">{t('comparison.totalCostComparison') || 'Total Cost Comparison'}</Label>
              </div>
            </div>
          </div>
          ...
          {comparison && (
            <div className="mt-6 space-y-6">
              <h3 className="font-medium mb-2">{t('comparison.results') || 'Comparison Results'}</h3>
                ...
                <table className="w-full border-collapse">
                  <thead>
                    <tr className="bg-gray-100">
                      ...
                      <th className="p-2 text-right">{t('comparison.monthlyPayment') || 'Monthly Payment'}</th>
                      <th className="p-2 text-right">{t('comparison.totalInterest') || 'Total Interest'}</th>
                      <th className="p-2 text-right">{t('comparison.term') || 'Term'}</th>
                      <th className="p-2 text-right">{t('comparison.totalCost') || 'Total Cost'}</th>
                    </tr>
                  </thead>
                  <tbody>
              ...
              {comparison.differences.length > 0 && (
                <div className="mt-4 p-4 bg-gray-50 rounded-md">
                  <h4 className="font-medium mb-2">{t('comparison.differences') || 'Differences'}</h4>
              ...
              {comparison.breakEvenPoint && options.includeBreakEvenAnalysis && (
                <div className="mt-4 p-4 bg-blue-50 rounded-md">
                  <h4 className="font-medium mb-2">{t('comparison.breakEvenPoint') || 'Break-even Point'}</h4>
                  <p>
                    {t('comparison.breakEvenDescription') || 'The scenarios break even at'}:
                    <span className="ml-2 font-medium">
                      {t('comparison.month') || 'Month'} {comparison.breakEvenPoint}
                      ({Math.floor(comparison.breakEvenPoint / 12)} {t('form.years') || 'years'}, {comparison.breakEvenPoint % 12} {t('comparison.months') || 'months'})
                    </span>
                  </p>
                </div>
              )}
