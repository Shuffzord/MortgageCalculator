...
describe('Repayment Model Tests', () => {
  test('RM1: Equal Installments Model (Annuity)', () => {
    ...
    expect(Math.abs(schedule[0].monthlyPayment - schedule[schedule.length - 2].monthlyPayment)).toBeLessThan(0.1);
    ...
    expect(schedule[schedule.length / 2].principalPayment).toBeGreaterThan(schedule[0].principalPayment);
  ...
  test('RM2: Decreasing Installments Model', () => {
    ...
    expect(schedule[0].monthlyPayment).toBeGreaterThan(schedule[schedule.length - 2].monthlyPayment);
    ...
    expect(Math.abs(schedule[schedule.length / 2].principalPayment - principalPortion)).toBeLessThan(0.1);
  ...
  test('RM3: Compare Equal vs Decreasing Installments', () => {
    ...
    expect(decreasingResults.amortizationSchedule[0].monthlyPayment)
      .toBeGreaterThan(equalResults.amortizationSchedule[0].monthlyPayment);
