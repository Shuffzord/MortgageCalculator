  ...
  for (let i = 1; i < scenariosWithResults.length; i++) {
    ...
    differences.push({
      ...
      monthlyPaymentDiff: roundToCents(
        base.results.monthlyPayment - current.results.monthlyPayment
      ),
      termDiff: base.results.actualTerm - current.results.actualTerm,
      totalCostDiff: roundToCents(
        base.results.totalInterest +
          (base.results.oneTimeFees || 0) +
          (base.results.recurringFees || 0) -
          (current.results.totalInterest +
            (current.results.oneTimeFees || 0) +
            (current.results.recurringFees || 0))
      ),
    });
  }
 ...
 */
export function calculateBreakEvenPoint(schedule1: any[], schedule2: any[]): number | undefined {
 ...
 */
export function calculateCumulativeCostDifference(schedule1: any[], schedule2: any[]): number[] {
 ...
 */
export function calculateMonthlyPaymentDifference(schedule1: any[], schedule2: any[]): number[] {
  const maxLength = Math.max(schedule1.length, schedule2.length);
  const differences: number[] = [];

  for (let i = 0; i < maxLength; i++) {
    const payment1 =
      i < schedule1.length ? schedule1[i].monthlyPayment + (schedule1[i].fees || 0) : 0;
    const payment2 =
      i < schedule2.length ? schedule2[i].monthlyPayment + (schedule2[i].fees || 0) : 0;
