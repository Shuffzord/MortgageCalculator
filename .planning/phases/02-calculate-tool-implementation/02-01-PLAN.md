...
**3. mapInputToLoanDetails function:**
- Convert validated input to LoanDetails format
...
- Use current date for startDate, generic name for name field
...
- On validation failure: Return FIRST error only with field path (per CONTEXT.md)
  ...
  ```
- On success: Map input -> LoanDetails -> calculateLoanDetails() -> formatResponse()
...
2. calculateMortgageTool is exported from @/lib/webmcp
3. inputSchema follows JSON Schema specification
4. Tool uses Zod safeParse for non-throwing validation
5. Error responses include field path and message
6. Success responses contain raw numbers (no formatted strings)
7. calculationEngine.calculateLoanDetails is called with proper LoanDetails mapping
</verification>
...
- [ ] Input mapped to LoanDetails with interestRatePeriods array
